...
def engadgeten_grab():
    ...
    for news in news_list:
        try:
            item = {}
            item['title'] = news.find('span').text.strip()
            item['url'] = 'http://www.engadget.com' + news.find('a', {'class': 'o-hit__link'}).get('href')
            time = news.find_all('span', attrs={'class': ''})[1].text.strip()
            ...
            pass
    return item, News.SOURCE_ENGADGETEN